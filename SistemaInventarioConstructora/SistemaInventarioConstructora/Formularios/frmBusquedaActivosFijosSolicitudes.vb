...
Public Class frmBusquedaActivosFijosSolicitudes


    Private Sub frmBusquedaActivosFijosSolicitudes_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LlenarDatos()
        txtBuscar.Focus()
    End Sub
    ...
    Sub LlenarDatos()
        Dim sql As String
        sql = "SELECT CargoActivos.IdCargo, CargoActivos.CodigoInventario, Articulos.NombreA, Empleados.Nombre, Empleados.Identidad, Departamentos.NombreD  ,CargoActivos.FechaAsignacion , Articulos.IdArticulo, Empleados.IdEmpleado, CargoActivos.Descripcion, Articulos.CodigoA, CargoActivos.EstadoArticulo
    ...
    Sub BuscarDatos()
        If rbNombreEmpleado.Checked Then
            If txtBuscar.Text = "" Then
                LlenarDatos()
            End If
            adaptador = New SqlDataAdapter("SELECT CargoActivos.IdCargo, CargoActivos.CodigoInventario, Articulos.NombreA,  Empleados.Nombre, Empleados.Identidad, Departamentos.NombreD  ,CargoActivos.FechaAsignacion , Articulos.IdArticulo, Empleados.IdEmpleado, CargoActivos.Descripcion, Articulos.CodigoA, CargoActivos.EstadoArticulo
           FROM Articulos INNER JOIN CargoActivos ON Articulos.IdArticulo = CargoActivos.IdArticulo
                          INNER JOIN Empleados ON CargoActivos.IdEmpleado = Empleados.IdEmpleado
    ...
    Private Sub dgvActivosAsignados_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvActivosAsignados.CellDoubleClick
        ...
        frmRegistroSolicitudes.txtIdArticulo.Text = CStr(dgvActivosAsignados.Item("IdArticulo", dgvActivosAsignados.CurrentCell.RowIndex).Value)
        frmRegistroSolicitudes.txtNombreA.Text = CStr(dgvActivosAsignados.Item("NombreA", dgvActivosAsignados.CurrentCell.RowIndex).Value)
        frmRegistroSolicitudes.txtCodigoI.Text = CStr(dgvActivosAsignados.Item("CodigoInventario", dgvActivosAsignados.CurrentCell.RowIndex).Value)


        frmRegistroSolicitudes.txtIdCargo.Text = CStr(dgvActivosAsignados.Item("IdCargo", dgvActivosAsignados.CurrentCell.RowIndex).Value)
        frmRegistroSolicitudes.txtNombreE.Text = CStr(dgvActivosAsignados.Item("Nombre", dgvActivosAsignados.CurrentCell.RowIndex).Value)
        frmRegistroSolicitudes.txtCodigoI.Text = CStr(dgvActivosAsignados.Item("CodigoInventario", dgvActivosAsignados.CurrentCell.RowIndex).Value)
        frmRegistroSolicitudes.txtDepartamento.Text = CStr(dgvActivosAsignados.Item("NombreD", dgvActivosAsignados.CurrentCell.RowIndex).Value)
